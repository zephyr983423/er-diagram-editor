...
export class ERDiagramApp {
    ...
    updatePropertiesPanel() {
        ...
        if (this.state.selectedItems.length === 0) {
            panel.innerHTML = '<p class="no-selection">Aucune sélection</p>';
        } else if (this.state.selectedItems.length === 1) {
            const item = this.state.selectedItems[0];
            if (item.type === 'entity') {
                this.renderEntityProperties(item);
            } else if (item.type === 'association') {
                this.renderAssociationProperties(item);
            } else if (item.type === 'connection') {
                this.renderConnectionProperties(item);
            }
        } else {
            panel.innerHTML = `<p class="multi-selection">${this.state.selectedItems.length} éléments sélectionnés</p>`;
        }
    }
    ...
    renderEntityProperties(entity) {
        ...
        panel.innerHTML = `
            <div class="property-section">
                <h3>Entité: ${entity.name}</h3>
                <button class="btn btn-primary" onclick="window.app.modalManager.openEntityModal('${entity.id}')">
                    Modifier
                </button>
            </div>
            <div class="property-section">
                <h4>Attributs (${entity.attributes.length})</h4>
                <ul class="attribute-list">
                    ${entity.attributes.map(attr => {
                        const badges = [];
                        if (attr.isPK) badges.push('<span class="badge badge-pk">PK</span>');
                        if (attr.isUQ) badges.push('<span class="badge badge-uq">UQ</span>');
                        if (!attr.isNull) badges.push('<span class="badge badge-nn">NOT NULL</span>');
                        return `<li>${attr.name} <span class="attr-type">${attr.type}</span> ${badges.join(' ')}</li>`;
                    }).join('')}
                </ul>
            </div>
        `;
    ...
    renderAssociationProperties(assoc) {
        const panel = document.getElementById('properties-content');
        const connections = this.state.getConnectionsForAssociation(assoc.id);

        panel.innerHTML = `
            <div class="property-section">
                <h3>Association: ${assoc.name}</h3>
                <button class="btn btn-primary" onclick="window.app.modalManager.openAssociationModal('${assoc.id}')">
                    Modifier
                </button>
            </div>
            <div class="property-section">
                <h4>Connexions (${connections.length})</h4>
                <ul class="connection-list">
                    ${connections.map(conn => {
                        const entity = this.state.getEntity(conn.entityId);
                        return `<li>${entity?.name || 'Entité inconnue'} <span class="cardinality">${conn.cardinality}</span></li>`;
                    }).join('')}
                </ul>
            </div>
            ${assoc.attributes.length > 0 ? `
                <div class="property-section">
                    <h4>Attributs (${assoc.attributes.length})</h4>
                    <ul class="attribute-list">
                        ${assoc.attributes.map(attr => `<li>${attr.name} <span class="attr-type">${attr.type}</span></li>`).join('')}
                    </ul>
                </div>
            ` : ''}
        `;
    ...
    renderConnectionProperties(conn) {
        ...
        panel.innerHTML = `
            <div class="property-section">
                <h3>Connexion</h3>
                <p><strong>Association:</strong> ${assoc?.name || 'Inconnue'}</p>
                <p><strong>Entité:</strong> ${entity?.name || 'Inconnue'}</p>
                <p><strong>Cardinalité:</strong> ${conn.cardinality}</p>
                ${conn.label ? `<p><strong>Étiquette:</strong> ${conn.label}</p>` : ''}
            </div>
        `;
    }
